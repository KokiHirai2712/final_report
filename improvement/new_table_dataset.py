...
def main() -> None:
    ...
    class_mapping: Dict[str, int] = {label:idx for idx, label in enumerate(np.unique(df['class']))}
    df['class'] = df['class'].map(class_mapping)    # クラスラベルを整数にエンコーディング

    features: torch.Tensor = torch.tensor(df[['sepal-length', 'sepal-width', 'petal-length', 'petal-width']].values,
                            dtype=torch.float)  # 説明変数のTensor

    labels: torch.Tensor = torch.tensor(df['class'].values, dtype=torch.long)     # 目的変数のTensor
    ...
    ## 損失関数とオプティマイザーの設定
    criterion: nn.modules.loss._Loss = nn.CrossEntropyLoss()                   # 損失関数（ソフトマックス交差エントロピー）
    optimizer: optim.Optimizer = optim.Adam(net.parameters(), lr=0.001)  # オプティマイザー（Adamオプティマイザー）
    ...
    for epoch in range(epoch_size):     # エポックのループ
        ...
        train_acc: List[float] = accuracy_score(train_true, train_pred)      # トレーニングでの正答率をsklearnの機能で算出
    ...
    test_true = []
    test_pred = []
    for istr, data in enumerate(test_loader):  # バリデーションのループ
        features, labels = data
